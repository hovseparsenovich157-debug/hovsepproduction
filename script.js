...
postersRow.addEventListener('touchend', (e) => {
  let endX = e.changedTouches[0].clientX;
  let diff = startX - endX;
  const cardWidth = postersRow.querySelector('.poster-item').offsetWidth + 16;
...
// Кнопки "‹ ›"
document.querySelector('.rail-btn.prev').addEventListener('click', () => {
  postersRow.scrollBy({ left: -260, behavior: 'smooth' });
});
document.querySelector('.rail-btn.next').addEventListener('click', () => {
  postersRow.scrollBy({ left: 260, behavior: 'smooth' });
});
