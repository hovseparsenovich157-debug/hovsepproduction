...
// --- проверка: учитывать ли визит ---
function shouldCountVisit() {
  // Если в вашем браузере заранее выполнено:
  // localStorage.setItem("dontCountMe", "1")
  if (localStorage.getItem("dontCountMe") === "1") return false;

  // Временно отключить: добавить ?skipcount=1 в URL
  const params = new URLSearchParams(window.location.search);
  if (params.get("skipcount") === "1") return false;
...
async function updateCounter() {
  ...
  // инкрементируем только если это не вы
  if (shouldCountVisit()) {
    await Promise.all([
